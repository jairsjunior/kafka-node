...
//SASL Mechanism
const oauth = require('../oauth/oauthClient');
...
async function encodeSaslAuthenticateRequest (clientId, correlationId, apiVersion, saslOpts) {
  //
  ...
  if (saslOpts.mechanism.toUpperCase() === 'PLAIN') {
    authBytes =
      (new Buffermaker())
        ...
        .string(password)
        .make();
  } else if (saslOpts.mechanism.toUpperCase() === 'OAUTHBEARER') {
    let oauthData = await oauth.mountJWT(saslOpts);
    authBytes = (new Buffermaker())
    .string('n,,')
    .Int8(1)
    .string(`auth=Bearer ${oauthData.value}`)
    .Int8(1)
    .Int8(1)
    .make();
  } else {
